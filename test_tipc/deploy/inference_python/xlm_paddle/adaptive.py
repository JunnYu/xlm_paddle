...
from paddle.nn import LayerList, Linear, Sequential
...
class AdaptiveLogSoftmaxWithLoss(nn.Layer):
    ...
    * :attr:`head_bias` if set to True, adds a bias term to the 'head' of the
    ...
    def __init__(
        ...
        if ((cutoffs != sorted(cutoffs)) or (min(cutoffs) <= 0) or
            (max(cutoffs) >
             (n_classes - 1)) or (len(set(cutoffs)) != len(cutoffs)) or
    ...
    def forward(self, input: Tensor, target: Tensor) -> _ASMoutput:
        ...
        for i in range(len(cutoff_values) - 1):
            ...
            if i == 0:
                ...
                scatter_output = paddle.scatter_nd(
                    ...
                    gather_inds.shape, )
                gather_inds = gather_inds * (scatter_output == 0
                                             ) + scatter_output
                ...
                local_logprob = (
                    (one_hot(relative_target, cluster_logprob.shape[-1]) *
        ...
        output += (
            paddle.nn.functional.one_hot(gather_inds, head_logprob.shape[1]) *
            head_logprob).sum(1)
        loss = (-output).mean()
