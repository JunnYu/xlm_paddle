...
class Dictionary(object):
    ...
    def check_valid(self):
        ...
        assert all(self.id2word[4 + i] == SPECIAL_WORD % i
    ...
    def max_vocab(self, max_vocab):
        ...
        self.counts = {
            k: v
            for k, v in self.counts.items() if k in self.word2id
        }
        self.check_valid()
        logger.info(
    ...
    def min_count(self, min_count):
        ...
        self.id2word = {
            ...
            if self.counts[self.id2word[k]] >= min_count or k < 4 +
            SPECIAL_WORDS
        }
        self.word2id = {v: k for k, v in self.id2word.items()}
        self.counts = {
    ...
    def read_vocab(vocab_path):
        ...
        for i, line in enumerate(f):
            ...
            if not line[1].isdigit():
                ...
                continue
            word2id[line[
                0]] = 4 + SPECIAL_WORDS + i - skipped  # shift because of extra words
        ...
        for i, line in enumerate(f):
            ...
            for w in s:
                ...
                if 0 <= word_id < 4 + SPECIAL_WORDS and word_id != 3:
                    logger.warning('Found unexpected special word "%s" (%i)!!'
                                   % (w, word_id))
                    continue
                assert word_id >= 0
